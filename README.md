<div align="center">

```
███╗   ███╗ ██████╗ ██╗  ██╗ █████╗ ███╗   ███╗███╗   ███╗███████╗██████╗
████╗ ████║██╔═══██╗██║  ██║██╔══██╗████╗ ████║████╗ ████║██╔════╝██╔══██╗
██╔████╔██║██║   ██║███████║███████║██╔████╔██║██╔████╔██║█████╗  ██║  ██║
██║╚██╔╝██║██║   ██║██╔══██║██╔══██║██║╚██╔╝██║██║╚██╔╝██║██╔══╝  ██║  ██║
██║ ╚═╝ ██║╚██████╔╝██║  ██║██║  ██║██║ ╚═╝ ██║██║ ╚═╝ ██║███████╗██████╔╝
╚═╝     ╚═╝ ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝╚═╝     ╚═╝╚══════╝╚═════╝
```

**`BREMOO`**

</div>

---

<img align="right" width="300" src="https://raw.githubusercontent.com/platane/snk/output/github-contribution-grid-snake-dark.svg" alt="snake animation"/>

### Hi there, I'm Mohammed 👋

CS Senior at Islamic University of Madinah, building things that **think** and **move**.

I work at the intersection of **Generative AI**, **autonomous agents**, and **robotics** — obsessed with systems that operate with minimal human intervention. From local RAG pipelines to combat robots, I gravitate toward problems where intelligence meets hardware.

Currently finishing my degree while publishing research, shipping products, and winning hackathons.

📍 **Madinah, Saudi Arabia** &nbsp;|&nbsp; 🎓 **Islamic University** &nbsp;|&nbsp; 🤖 **AI × Robotics**

<br clear="right"/>

---

## 🧠 What I Build

> *"Autonomous systems that don't wait to be told what to do."*

```python
class Mohammed:
    role     = "AI Developer & CS Senior"
    focus    = ["Generative AI", "LLM Agents", "RAG Systems", "Combat Robotics"]
    location = "Madinah 🇸🇦"
    status   = "Building things that think"

    def current_obsession(self):
        return "Making LLMs work reliably in production"
```

---

## 📊 GitHub Activity

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Moha-Bremoo&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=58A6FF&icon_color=58A6FF&text_color=C9D1D9" height="165" alt="GitHub Stats"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Moha-Bremoo&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=58A6FF&text_color=C9D1D9" height="165" alt="Top Languages"/>
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Moha-Bremoo&theme=tokyonight&hide_border=true&background=0D1117&stroke=58A6FF&ring=58A6FF&fire=FF6B6B&currStreakLabel=58A6FF" alt="GitHub Streak"/>
</div>

---

## 🛠️ Tech Stack

**Core Languages**

![Python](https://img.shields.io/badge/Python-0D1117?style=for-the-badge&logo=python&logoColor=3776AB)
![C++](https://img.shields.io/badge/C++-0D1117?style=for-the-badge&logo=c%2B%2B&logoColor=00599C)
![Java](https://img.shields.io/badge/Java-0D1117?style=for-the-badge&logo=openjdk&logoColor=ED8B00)
![SQL](https://img.shields.io/badge/SQL-0D1117?style=for-the-badge&logo=mysql&logoColor=4479A1)

**AI / ML**

![PyTorch](https://img.shields.io/badge/PyTorch-0D1117?style=for-the-badge&logo=pytorch&logoColor=EE4C2C)
![TensorFlow](https://img.shields.io/badge/TensorFlow-0D1117?style=for-the-badge&logo=tensorflow&logoColor=FF6F00)
![Keras](https://img.shields.io/badge/Keras-0D1117?style=for-the-badge&logo=keras&logoColor=D00000)
![LangChain](https://img.shields.io/badge/LangChain-0D1117?style=for-the-badge&logo=chainlink&logoColor=1C3C3C)

**LLM Ecosystem**

![Ollama](https://img.shields.io/badge/Ollama-0D1117?style=for-the-badge&logo=llama&logoColor=white)
![Gemini](https://img.shields.io/badge/Gemini-0D1117?style=for-the-badge&logo=google&logoColor=4285F4)
![OpenAI](https://img.shields.io/badge/OpenAI-0D1117?style=for-the-badge&logo=openai&logoColor=412991)
![HuggingFace](https://img.shields.io/badge/HuggingFace-0D1117?style=for-the-badge&logo=huggingface&logoColor=FFD21E)

**Web & Tools**

![Flask](https://img.shields.io/badge/Flask-0D1117?style=for-the-badge&logo=flask&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-0D1117?style=for-the-badge&logo=bootstrap&logoColor=7952B3)
![Git](https://img.shields.io/badge/Git-0D1117?style=for-the-badge&logo=git&logoColor=F05032)
![Linux](https://img.shields.io/badge/Linux-0D1117?style=for-the-badge&logo=linux&logoColor=FCC624)

---

## 📄 Research & Publications

<table>
<tr>
<td width="50">📑</td>
<td>

**AutoArabic: A Three-Stage Framework for Localizing Video-Text Retrieval Benchmarks**
<br>*ArabicNLP 2025 · ACL Anthology*
<br>Benchmark localization pipeline for Arabic video-text retrieval — addressing the gap in Arabic multimodal NLP resources.

</td>
</tr>
<tr>
<td width="50">📑</td>
<td>

**Multimodal Lengthy Videos Retrieval Framework and Evaluation Metric**
<br>*arXiv preprint · 2025*
<br>A retrieval framework and novel evaluation metric designed specifically for long-form video understanding.

</td>
</tr>
</table>

---

## 🚀 Selected Projects

<table>
<tr>
<td width="50%">

### 🌟 The Star Weavers
**1st Place — NASA Hackathon (Youth Changemakers Awards)**

An AI generative pipeline built for NASA's space challenge. End-to-end creative generation system combining LLMs with multimedia output.

</td>
<td width="50%">

### 💰 Mustasharak
**Intelligent Financial Planning for Saudi Arabia**

Hybrid architecture using Google Gemini + local LLMs for personalized financial guidance — Sharia-compliant, Arabic-first.

</td>
</tr>
<tr>
<td width="50%">

### 🌧️ AeroSeed
**AI-Driven Cloud Seeding Optimization**

LightGBM + Quantile Regression model for weather modification decision-making. Achieved **19% error reduction** over baselines.

</td>
<td width="50%">

### 📋 Formal Letter Generator
**Regulation-Aware Document Automation**

Automatic generation of precise formal letters aligned with uploaded legal documents and institutional regulations.

</td>
</tr>
</table>

---

## 📫 Find Me

<div align="left">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Mohammed_Bremoo-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mohammed-bremoo/)
[![X](https://img.shields.io/badge/X-@M__Bremoo-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/M_Bremoo)

</div>

---

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=Moha-Bremoo&style=for-the-badge&color=58A6FF&label=PROFILE+VIEWS" alt="Profile views"/>
</div>

<div align="center">
  <sub>Always building. Always learning.</sub>
</div>
